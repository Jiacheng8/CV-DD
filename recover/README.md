# Usage
```
Usage: Recover data from pre-trained model using CV-DD or Sre2L++.
...
# Running on Low-Memory GPUs

If you are using a GPU with limited memory (less than 30GB), you may encounter a `CUDA_OUT_OF_MEMORY` error.  
To address this issue while maintaining the quality of the distilled dataset, we recommend using the modified version of `recover.py` provided below.
```python
import argparse
import collections
import os
...
This adaptation enables training on smaller-memory GPUs **without compromising performance**.
